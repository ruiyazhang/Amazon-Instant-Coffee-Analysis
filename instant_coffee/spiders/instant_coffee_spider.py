...
class InstantCoffeeSpider(Spider):
    ...
    def parse_results_page(self, response):
        ...
        rating_avgs = response.xpath('//div[@class="s-main-slot s-result-list s-search-results sg-row"]/div[not(contains(@class, "AdHolder"))]//div[@class="s-expand-height s-include-content-margin s-border-bottom s-latency-cf-section"]/div[1]/div[@class="a-section a-spacing-none a-spacing-top-micro"]/div[1]/span[1]/@aria-label').extract()
        rating_nums = response.xpath('//div[@class="s-main-slot s-result-list s-search-results sg-row"]/div[not(contains(@class, "AdHolder"))]//div[@class="s-expand-height s-include-content-margin s-border-bottom s-latency-cf-section"]/div[1]/div[@class="a-section a-spacing-none a-spacing-top-micro"]/div[1]/span[2]/@aria-label').extract()
        ...
        for index in range(5):
            ...
            meta = {
                ...
                'rating_avg': rating_avgs[index],
                'rating_num': rating_nums[index]
            }
    ...
    def parse_product_page(self, response):
        ...
        if not brand:
            brand=response.xpath('//div[@id="bylineInfoUS_feature_div"]/div[@class="a-section a-spacing-none"]/text()').extract_first()
            brand=str.strip(brand)
            # brand=(re.findall("^.{1,7}(.*)$",brand))[0]
        ...
        unit_price = unit_price_first_try or unit_price_second_try or unit_price_third_try or unit_price_fourth_try or unit_price_fifth_try
        ...
        top_sales_rank = response.xpath('//li[@class="zg_hrsr_item"]/span[@class="zg_hrsr_rank"]/text()').extract_first()
        ...
        rating_avg = None
        try:
            rating_avg = float((re.findall('\d+\.\d+',response.meta['rating_avg']))[0])
        except:
            None
        ...
        print(unit_price)
        print(ASIN)
        print(top_sales_rank)
        print(response.meta['product_name'])
        print(rating_avg)
        print(rating_num)
        print('='*50)
